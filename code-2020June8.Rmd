...
NCDB_lung$UR_CD_03_2=ifelse(NCDB_lung$UR_CD_03 %in% c(1, 2, 3), 'Metro', 
                            ifelse(NCDB_lung$UR_CD_03 %in% c(4, 5, 6, 7), 'Urban', 
                                   ifelse(NCDB_lung$UR_CD_03 %in% c(8, 9), 'Rural',
                                          'Unknown')))
...
TUMOR_SIZE_cat + AGE_cat + LYMPH_VASCULAR_INVASION2 + Visceral_Pleural_Invasion + GRADE + CDCC_TOTAL_BEST + Neuroendocrine + Margins + Pathology + INSURANCE_STATUS
...
fit_mort0=glm(PUF_VITAL_STATUS~1, 
             data=na.omit(NCDB_lung[,include_columns_mort]), 
             family = binomial(link="logit"))
mort_forward=step(fit_mort0, scope=list(lower=formula(fit_mort0),upper=formula(fit_mort)),
                  direction="forward")
```
...
cox_fit <- coxph(Surv(DX_LASTCONTACT_DEATH_MONTHS,PUF_VITAL_STATUS) ~ Chemo+
                      AGE_cat+       
                      SEX +                     
                      CDCC_TOTAL_BEST  +         
                      TUMOR_SIZE_cat  +          
...
```{r}
#write final data : best predictors of mortality
lung_final=NCDB_lung[, include_columns3]
write.csv(lung_final,"lung_data2.csv", row.names = F)
```
...
Factors: - Chemo - High-risk - Interaction term
...
cph_fit_chemo = cph(
  Surv(DX_LASTCONTACT_DEATH_MONTHS, PUF_VITAL_STATUS==0) ~
                      ...
                      Lymph_Nodes_Sampled +
                      Excision_less_than1,
                    data = NCDB_lung2, surv = TRUE, x=TRUE, y=TRUE,
  time.inc=24,subset=Chemo=="Chemo")
...
#plot(NCDB_lung2$DX_LASTCONTACT_DEATH_MONTHS[1:100], des1$surv[1:100])
```

```{r}
#write.csv(NCDB_lung2,"lung2.csv")
write.csv(NCDB_lung2,"lungdata.csv")
lung<-read.csv("lungdata.csv")
```

```{r}
require(muhaz)
s<-muhaz(lung$DX_LASTCONTACT_DEATH_MONTHS, lung$PUF_VITAL_STATUS)
exp(s$haz.est)
```
...
cph_fit = cph(
  Surv(DX_LASTCONTACT_DEATH_MONTHS, PUF_VITAL_STATUS) ~  Chemo +
                      ...
                      Lymph_Nodes_Sampled +    
                      Excision_less_than1 , 
                    data = lung, surv = TRUE, x=TRUE, y=TRUE, time.inc=24)
...
```{r}
cph_fit2 = cph(
  Surv(DX_LASTCONTACT_DEATH_MONTHS, PUF_VITAL_STATUS==1) ~  Chemo,
                      data = lung, surv = TRUE, x=TRUE, y=TRUE, time.inc=24)
surv200=survest(cph_fit2)

plot(exp(surv200$surv), surv200$cumhaz)
```

```{r}
# #Given the nomogram is the simpler method for the calculation
# of the risk score, we feel it is more likely to be remembered
# and therefore utilized in practice.
cph_fit_chemo = cph(
  Surv(DX_LASTCONTACT_DEATH_MONTHS, PUF_VITAL_STATUS==0) ~
                      AGE+
                      SEX +
                      ...
                      Excision_less_than1,
                    data = NCDB_lung2, 
  surv = TRUE, x=TRUE, y=TRUE,time.inc=24,
  subset=Chemo=="Chemo")

survkk<-survest(cph_fit_chemo)
risk<-exp(survkk$surv)
hr<-survkk$cumhaz #total hazard rate experienced adds up to 1
plot(risk, hr)


#----------------------
bs<-basehaz(cph_fit_chemo, centered = T) #BASELINE CURVE
plot(bs$time, bs$hazard)
```

```{r}
#-------RISK ESTIMATES-------------- 
#https://stats.stackexchange.com/a/376145
#To derive risk estimates from a Cox model you typically use the Breslow estimator for
#the cumulative baseline hazard that is coupled with the hazard ratios from the 
#Cox model. This is implemented in the survfit() function of the survival package. 
#In particular, if you give as a first argument your fitted Cox model, and in the 
#newdata arguments you specify a data frame with the specific values for the covariates
#X1, …, X4, you get survival probabilities, which you can transform to risks by 
#calculating 1 - Survival.
fit10<-survfit(cph_fit_chemo, newdata=NCDB_lung2)
risks<-1-fit10$surv
#------------------------------------------------

bh=basehaz(cph_fit_chemo, centered = FALSE) #baseline cumulative hazard function for all covariates set to zero(centered=False)
plot(bh)
length(bh$hazard) #945


lps=predict(cph_fit_chemo, NCDB_lung2, type = "lp") #lp=linear predictors, betas x covariates for each observation
length(lps) #10325
h_r= bh$hazard*(exp(lps)) #hazard_ratio
hr=exp(lps) #hazard ratio
time_of_interest=NCDB_lung2$DX_LASTCONTACT_DEATH_MONTHS #survival at time 6
# i_time_of_interest = match(1, round(bh$time, 1)==time_of_interest, 
#                            nomatch = -100)
# 
# event_prob_t=1-exp(-bh[i_time_of_interest, 1])^exp(lps)
...
# RISK SCORE FROM COXPH MODEL

```{r}
# #METHod 1 for hazard ratio 
# 
...
# #hazard ratio
require(survcomp)
hzr1<-hazard.ratio(x, surv.time, surv.event, weights, strat, alpha = 0.05,
method.test = c("logrank", "likelihood.ratio", "wald"), na.rm = FALSE, ...)

hzr1<-hazard.ratio(x=NCDB_lung2$AGE,
                   surv.time=NCDB_lung2$DX_LASTCONTACT_DEATH_MONTHS,
                   surv.event=NCDB_lung2$PUF_VITAL_STATUS==0)
...
library(ggplot2)
library(casebase)
# #HAZARD RATIO VS RISK SCORE
# #https://cran.r-project.org/web/packages/casebase/vignettes/plotsmoothHazard.html#hazard-ratio
library(TH.data)
data("brcancer")
str(brcancer)
...
#                           time = "time")
# 
hr2<-fitSmoothHazard(NCDB_lung2$PUF_VITAL_STATUS ~ NCDB_lung2$AGE +
                       NCDB_lung2$CDCC_TOTAL_BEST, data = NCDB_lung2,
                     time="time") #NCDB_lung2$DX_LASTCONTACT_DEATH_MONTHS
# 

table(NCDB_lung2$Chemo)
```

#check this

```{r}
#hazard ratio chemo vrs no chemo
cphchem<-cph(Surv(DX_LASTCONTACT_DEATH_MONTHS, PUF_VITAL_STATUS==1) ~ Chemo,
              data =NCDB_lung2,surv = TRUE, x=TRUE, y=TRUE,time.inc=24)

surv1<-survest(cphchem)
risk<-exp(surv1$surv)
hr<-surv1$cumhaz #total hazard rate experienced adds up to 1
plot(risk, hr)




lps=predict(cphchem, NCDB_lung2$DX_LASTCONTACT_DEATH_MONTHS, type = "lp")
length(lps) #10326
hr=exp(lps)  #hazard ratio

#since the hazard funtion can be expressed as the risk of dying at time t
#baseline cumulative hazard function for all covariates set to zero(centered=False)

# #risk score calculated for each patient
# based on the cumulative risk conferred from the 12 prognostic
# factors discussed previously.
model= cph(Surv(DX_LASTCONTACT_DEATH_MONTHS, PUF_VITAL_STATUS==0) ~ Chemo + 
                      AGE+
                      SEX +
                      CDCC_TOTAL_BEST  +
                      TUMOR_SIZE_cat  +
